...
object alu:
  ...
  private def dereference(args: List[Value]): Value =
    if (args.size != 1) throw new TypeException("1 input required by dereference")
    args(0) match {
      case b: Variable => b.content
      case _ => throw new TypeException("Input to dereference must be Variable")
    }

  private def makeVar(args: List[Value]): Value =
    if (args.size != 1) throw new TypeException("1 input required by makeVar")
    args(0) match {
      case b: Value => new Variable(b)
      case _ => throw new TypeException("Input to makeVar must be Variable")
    }

  private def write(args: List[Value]): Value = {
    println(args(0)); Notification.DONE
  }
